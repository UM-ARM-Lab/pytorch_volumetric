...
class URDFSDF(ObjectFrameSDF):
    ...
    def __init__(self, chain: pk.Chain, path_prefix='',
            ...
            for link_geom in link_geometries:
                if link_geom.geom_type not in supported_types:
                    ...
                    if link_geom.geom_type == "mesh":
                        ...
                        link_obj = MeshObjectFactory(link_geom.geom_param[0],
                                                         scale=link_geom.geom_param[1],
                                                         path_prefix=path_prefix)
                        link_sdf = link_sdf_cls(link_obj)
                    elif link_geom.geom_type == "box":
                        link_sdf = BoxSDF(link_geom.geom_param, device=self.device)
                    elif link_geom.geom_type == "cylinder":
                        link_sdf = CylinderSDF(link_geom.geom_param[0],
                                                   link_geom.geom_param[1], device=self.device)
                    elif link_geom.geom_type == "sphere":
                        link_sdf = SphereSDF(link_geom.geom_param, device=self.device)
                    else:
        ...
        self.sdf = ComposedSDF(sdfs, self.offset_transforms.inverse())
    ...
    def get_link_sdf(self, link_name) -> ObjectFrameSDF:
        """
