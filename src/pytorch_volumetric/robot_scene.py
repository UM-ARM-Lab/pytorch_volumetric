...
class RobotScene:
    ...
    def _generate_robot_query_points(self):
        ...
        for link_name in self.robot_sdf.sdf_to_link_name:
            ...
            points, _, _ = sdf.sample_mesh_points(link_sdf, self.points_per_link,
                                                dbpath=f'{self.rob_assets_path}/{link_name}_points_cache.pkl', device=self.device)
    ...
    def _generate_scene_query_points_only_left(self):
        query_points = []
        for link_name in self.robot_sdf.sdf_to_link_name:
        ...
        # mask out points that are in self-collision with default configuration
        tfs = self.robot_sdf.sdf.obj_frame_to_link_frame[2:10].inverse()
        # print("tfs set qpoints", tfs.get_matrix().shape, " tfs")
        pts = tfs.transform_points(query_points).reshape(-1, 3)
        # print("new sliced tfs pts", pts, pts.shape)
        sdf_vals, _ = self.robot_sdf(pts)
        mask = torch.where(sdf_vals < -self.threshold, torch.zeros_like(sdf_vals), torch.ones_like(sdf_vals))
        return query_points, mask
    ...
    def visualize_robot(self, q: torch.Tensor, only_left=False, cached=False):
        ...
        if only_left:
            left_tfs_indices = torch.arange(2, 10) + 18 * torch.arange(1).unsqueeze(-1)
            # print(left_tfs_indices, left_tfs_indices.shape, " left tfs indices")
    ...
    def scene_collision_check(self, q: torch.Tensor, only_left=False, compute_gradient=False, compute_hessian=False):
        ...
        # print(pts, pts.shape, " pre scene transform, post robot transform")
